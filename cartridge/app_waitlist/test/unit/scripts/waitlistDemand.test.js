...
describe('waitlistDemand', function () {
    describe('catalogStatus', function () {
        var mod;
        beforeEach(function () { mod = load([], {}).mod; });

        it('flags an unresolvable product as DELETED regardless of stock flag', function () {
            expect(mod.catalogStatus(false, false)).to.equal('DELETED');
            expect(mod.catalogStatus(true, false)).to.equal('DELETED'); // non-existence wins
        });
        it('reports IN_STOCK when a resolvable product is available', function () {
            expect(mod.catalogStatus(true, true)).to.equal('IN_STOCK');
        });
        it('reports OUT_OF_STOCK when a resolvable product is unavailable', function () {
            expect(mod.catalogStatus(false, true)).to.equal('OUT_OF_STOCK');
        });
    });
    ...
    describe('build', function () {
        ...
        it('ranks by raw waiting count desc, with catalog status only as a tie-break', function () {
            var rows = [
                co('IN', 'PENDING'),                                   // in stock, 1 waiting
                co('LOWDEM', 'PENDING'),                               // OOS, 1 waiting (ties IN)
                co('HIGHDEM', 'PENDING'), co('HIGHDEM', 'PENDING'),
                co('HIGHDEM', 'PENDING'), co('HIGHDEM', 'PENDING')     // OOS, 4 waiting
            ];
            ...
            // Count is king: 4 waiting first. On the 1-waiting tie, OUT_OF_STOCK
            // (needs attention) sorts ahead of IN_STOCK.
            expect(report.rows[0].sku).to.equal('HIGHDEM');
            expect(report.rows[0].waiting).to.equal(4);
            expect(report.rows[0].status).to.equal('OUT_OF_STOCK');
            expect(report.rows[1].sku).to.equal('LOWDEM');
            expect(report.rows[1].status).to.equal('OUT_OF_STOCK');
            expect(report.rows[2].sku).to.equal('IN');
            expect(report.rows[2].status).to.equal('IN_STOCK');
        });

        it('marks a SKU whose product no longer resolves as DELETED and not-in-stock', function () {
            ...
            expect(row.waiting).to.equal(1); // the count is retained independently
    ...
    describe('toCsv', function () {
        it('emits a header plus one row per SKU with RFC-4180 quoting', function () {
            ...
            expect(lines[0]).to.equal('SKU,Product,Waiting,Notified,Failed,Status');
            // Comma + embedded quotes force quoting with doubled inner quotes.
            expect(lines[1]).to.equal('SKU-1,"Fancy, ""Quoted"" Tee",1,0,0,OUT_OF_STOCK');
        });
    });
});
