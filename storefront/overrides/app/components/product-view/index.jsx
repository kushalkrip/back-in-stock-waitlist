...
// BACK-IN-STOCK: Notify Me form shown in place of Add-to-Cart when OOS.
// Relative import: this is a NEW component that exists only in overrides/, so
// the '@salesforce/retail-react-app/*' alias (which resolves to node_modules)
// can't find it.
...
const ProductView = forwardRef(
    (
        ...
        const renderActionButtons = () => {
            ...
            // ─── BACK-IN-STOCK ──────────────────────────────────────────────
            // Swap Add-to-Cart for the Notify Me form when the resolved
            // selection is out of stock. `isOutOfStock` comes from
            // useDerivedProduct and recomputes on every variant selection, so
            // switching to an in-stock size restores Add-to-Cart automatically.
            // For variation products we require a variant to be selected first
            // (so we send the VARIANT SKU, not the master id).
            const hasVariations = product?.variationAttributes?.length > 0
            const selectedSku = variant?.productId || product?.id
            // Real OOS only counts once a variant is resolved (so we send the
            // VARIANT sku, not the master). For variation products that means a
            // selection has been made; standard products are always resolved.
            const variantResolved = Boolean(variant?.productId) || !hasVariations
            // Demo helper: append `?forceOOS=1` to any PDP URL to preview the
            // Notify Me swap without needing a genuinely out-of-stock variant.
